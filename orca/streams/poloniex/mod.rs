...
pub fn connect(
    ...
    ws::connect(STREAM_URL, &handle)
        .and_then(move |stream| {
            let handle = ws::Handle::new(sender, commands, handle);
...
fn parse_body(body: OwnedMessage, stream: ws::Client, handle: ws::Handle) -> ws::LoopFuture {
    match body {
...
fn parse_message(body: String, stream: ws::Client, handle: ws::Handle) -> ws::LoopFuture {
    match self::parser::parse_message(&body) {
...
fn send_cmd(cmd: Command, stream: ws::Client, handle: ws::Handle) -> ws::LoopFuture {
    ws::send_msg(OwnedMessage::Text(self::cmd::serialize(cmd)), stream, handle)
}
